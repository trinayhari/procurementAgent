...
from app.schemas.rfq import PersistedRfq, RfqGenerateRequest, RfqUpdate
...
# --------------------------------------------------------------- RFQ generation
def _line_items_for_package(package: str) -> List[dict]:
    """Pull the BOM line items for a package from the project's extracted groups.

    Uses the shared seed LINE_ITEMS for now (same source the line-items endpoint
    serves); matches a group to the package via its label.
    """
    items: List[dict] = []
    for group in seed.LINE_ITEMS:
...
def generate_rfq(
    ...
    line_items = _line_items_for_package(package)
...
def send_generated_rfq(project_id: str, rfq_id: str, db: Session = Depends(get_db)):
    ...
    for r in recipients:
        try:
            mid = sender.send(r["email"], rfq["subject"], rfq["body"], from_addr=from_addr)
            r["sentMessageId"] = mid
        except Exception as exc:  # record the failure per-recipient, keep going
            r["sentMessageId"] = f"error: {exc}"

    return rfqs_repo.mark_rfq_sent(db, rfq_id, recipients)
