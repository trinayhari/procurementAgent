...
register(
    PlanTypeSpec(
        ...
        prompt_guidance=(
            ...
            "sheathing in SF.\n"
            "QUANTITIES ARE THE GOAL — this is a takeoff, so DERIVE a number for "
            "every item; do not default to null. Work each quantity from the "
            "drawings:\n"
            "  • SCHEDULES: read the QTY / count / mark columns directly — a footing, "
            "column, beam, or header schedule states how many of each mark. Use that "
            "count.\n"
            "  • PLANS: COUNT the tagged members and symbols shown (columns at grid "
            "intersections, posts, joists across a bay, headers over openings) "
            "against the legend/grid.\n"
            "  • DIMENSIONS: measure runs and areas from the dimensioned grid and the "
            ...
            "derived estimate. Use quantity null ONLY when there is genuinely no "
            "count, dimension, or area anywhere on the sheets to work from (e.g. an "
            "item shown only in a typical detail with no governing dimension, or one "
            "that explicitly defers to structural sheets not present in this set) — "
            "and say exactly why. Prefer a reasoned estimate with its assumptions "
            "over a blank. Deduplicate by mark/tag: the same footing or column "
            "appears on the plan, the schedule, AND a detail — count it ONCE (the "
            "schedule is authoritative for size/reinforcing, the plan for count). "
            "Expand 'TYP.'/typical details by the number of locations they apply to; "
