...
def build_user_prompt(spec: PlanTypeSpec, sheet: str = None) -> str:
    ...
    if sheet:
        return (
            ...
            "used) — extract only materials the notes explicitly require to be installed, "
            "with their stated rating/size (e.g. 'main service disconnect, min 150A').\n"
...
def build_consolidation_system_prompt() -> str:
    return (
        ...
        "element, and never emit a design alternate as a second line item. "
        "Return the final Bill of Materials."
    )
