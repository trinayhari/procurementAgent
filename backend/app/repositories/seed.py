...
DOC_LINE_ITEMS: Dict[str, List[dict]] = {}
_UPLOAD_SEQ = {"n": 0}
...
def set_document_line_items(doc_id: str, groups: List[dict]) -> None:
    DOC_LINE_ITEMS[doc_id] = groups


def save_document_line_items(doc_id: str, groups: List[dict]) -> List[dict]:
    """Persist a human-edited BOM: recompute counts/total, flag the doc as edited."""
    clean = [{**g, "count": len(g.get("items", []))} for g in groups]
    DOC_LINE_ITEMS[doc_id] = clean
    doc = get_document(doc_id)
    if doc is not None:
        total = sum(g["count"] for g in clean)
        doc["items"] = str(total)
        doc["edited"] = True
    return clean


def confirm_document(doc_id: str, when: str) -> Optional[dict]:
...
def get_document_line_items(doc_id: str) -> Optional[List[dict]]:
    """Per-document BOM groups: uploaded docs use their own store; the fleshed-out
    seed docs fall back to the shared LINE_ITEMS so the prototype still renders."""
    if doc_id in DOC_LINE_ITEMS:
        return DOC_LINE_ITEMS[doc_id]
    doc = get_document(doc_id)
    if doc and not doc.get("planType"):  # a seed doc, not an upload
        return LINE_ITEMS
    return None
