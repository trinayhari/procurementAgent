...
- **Secret hygiene problems**: live OpenRouter key, Google Maps key, and a
  Gmail OAuth **refresh token** sit in `apps/api/.env` in the working tree
...
### 2.1 Frontend (React + Vite + TS, `apps/web/src/`)
- Single-page app, hash-routed, gated behind JWT login ([App.tsx:307](../apps/web/src/App.tsx)).
- **Every major flow is wired to real API mutations** — this is not a mock UI:
  ...
  - BOM review: inline line-item editing (`PUT /line-items`), Confirm BOM
    (`POST /confirm`) — human-in-the-loop exists ([App.tsx:1134](../apps/web/src/App.tsx)).
  ...
  - Quote ingest + polling, quotes table, line-level comparison with three
    award strategies and split-award submit ([App.tsx:1883](../apps/web/src/App.tsx)).
  - Timeline milestones with done/overdue tracking.
- Type-safe client: `apps/web/src/api-types.ts` generated from the backend OpenAPI
  schema; CI fails on drift.
- Empty-state driven — `model.ts` falls back to empty arrays, not fake data
  (one dead hardcoded RFQ `thread` literal at [model.ts:296](../apps/web/src/model.ts) and unused
  `selectQuote` at [api.ts:372](../apps/web/src/api.ts)).

### 2.2 Document upload + BOM extraction (`apps/api/app/services/extraction/`)
...
- Runs in a **subprocess** with timeout to isolate PyMuPDF segfaults
  ([isolated.py](../apps/api/app/services/extraction/isolated.py)) — deliberate, battle-tested design.
...
### 2.3 Supplier discovery (`apps/api/app/services/sourcing/`)
...
### 2.4 RFQ + quotes (`apps/api/app/services/rfq/`, `quotes/`)
...
- Email: the codebase's one true provider abstraction — `EmailSender` Protocol
  ...
  `MockSender` ([sender.py:41](../apps/api/app/services/rfq/sender.py)).
...
- Comparison: weighted recommendation (0.6 cost / 0.3 lead / 0.1 risk) plus a
  ...
  award strategies ([line_comparison.py:63](../apps/api/app/services/quotes/line_comparison.py)).
...
| No organizations/tenants; `Project` has no owner column; queries never scoped to caller | [models/project.py](../apps/api/app/models/project.py), [models/user.py](../apps/api/app/models/user.py) | Critical |
| Demo account `jordan@meridiancivil.com`/`procureai` seeded unconditionally in every deploy | [repositories/users.py:68](../apps/api/app/repositories/users.py), [db.py:55](../apps/api/app/db.py) | Critical |
| Default JWT secret `dev-insecure-change-me`, no startup guard | [config.py:34](../apps/api/app/config.py) | Critical |
| Live secrets in working-tree `apps/api/.env` (OpenRouter key, Maps key, **Gmail refresh token**) — rotate all | `apps/api/.env` | Critical (operational) |
| Uploads collide by filename (`uploads/<name>`, no namespacing); no extension allowlist; direct unauthenticated file download route | [documents.py:184](../apps/api/app/api/routes/documents.py) | High |
| No rate limiting, no roles/permissions, no trusted-host, 7-day tokens, no revocation | — | High |
| CORS hardcoded to localhost origins with credentials on | [config.py:13](../apps/api/app/config.py), [main.py:42](../apps/api/app/main.py) | Medium |

### 3.2 Reliability & durability
| Issue | Location | Severity |
|---|---|---|
| Supplier-search and quote-ingest status in **module-level dicts** — lost on restart, wrong under >1 worker | [sourcing.py:46,49](../apps/api/app/api/routes/sourcing.py) | Critical |
| **RFQ send has no idempotency, no status precondition, no duplicate-send guard** — re-POST re-emails all suppliers | [sourcing.py:460](../apps/api/app/api/routes/sourcing.py) | Critical |
| No retry for failed sends — failure stored as `sentMessageId="error: ..."` string | [sourcing.py:483](../apps/api/app/api/routes/sourcing.py) | High |
...
| Status strings with **no state-machine enforcement** (Draft→Sent→Awaiting→Quoted set freely) | [models/rfq.py:32](../apps/api/app/models/rfq.py), repos | High |
...
| `init_db()` runs `create_all` + manual `ALTER TABLE`s alongside Alembic — schema drift (already bit us locally) | [db.py:35,71](../apps/api/app/db.py) | Medium |
...
- **No ProcurementPackage entity** — packages are a hardcoded 15-category
  preset dict ([packages.py:19](../apps/api/app/services/sourcing/packages.py)); no CRUD, no per-package delivery
...
- Default SQLite DB; a stale `apps/api/procureai.db` (258 KB) and an empty root
...
- `POST /api/rfqs/{id}/messages` and `/followup` are stubs operating on demo
  reference RFQs, not real ones ([rfqs.py:34](../apps/api/app/api/routes/rfqs.py)).
- Mock quotes carry no line items → line comparison silently empty on the mock
  path ([ingest.py:87](../apps/api/app/services/quotes/ingest.py)).
- `_recipient_index` maps first-RFQ-per-email — a supplier on two package RFQs
  only ingests against one ([ingest.py:26](../apps/api/app/services/quotes/ingest.py)).
- Places Text Search reads only page 1 of results ([places.py:63](../apps/api/app/services/sourcing/places.py));
  distances are haversine, not driving.
- Award/"issue PO" is a status flip + event string — no PO/PurchaseDecision
  record ([projects.py:174](../apps/api/app/api/routes/projects.py)).
...
- Committed junk: `.DS_Store` files, `apps/api/procureai.db`, root `procureai.db`.
