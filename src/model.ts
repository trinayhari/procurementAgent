import { tone, badge, chip, bar, ic, lb } from './lib.jsx'
...
export function buildModel(s, set, props) {
  // Raw data fetched from the backend (see src/api.js). Each section below falls
  // back to its baked-in literal when a key is absent, so the UI renders fully
  // even if the API is unavailable.
  const D = (props && props.data) || {}
  const go = (n) => set({ nav: n, mnav: false, compare: false, supplierId: null })
  const setTab = (t) => set({ tab: t, compare: false, supplierId: null })
  ...
  const metrics = metricsRaw.map((m) => {
    ...
    return { ...m, deltaStyle: { display: 'inline-flex', alignItems: 'center', gap: 3, color: c } }
  })
  ...
  const extracted = (perDoc || D.lineItems || [
    ...
    return { ...g, dotStyle: { width: 8, height: 8, borderRadius: 2, background: fg, flex: 'none' }, countBadge: badge(g.tone, { fontSize: 11, padding: '1px 8px' }) }
  })
  ...
  ]).map((q) => ({ ...q, onOpen: () => set({ compare: true }), logoStyle: lb(q.logoBg, 30) }))
  ...
  const cmpRowsRaw = (D.comparison && D.comparison.rows) || [
  ...
  return {
    ...
    docs, doc, extracted,
    // Upload / extraction wiring (see App.jsx + TabDocuments).
    planTypes: (props && props.planTypes) || null,
    planType: (props && props.planType) || 'site_plan',
    setPlanType: (key) => set({ planType: key }),
    uploading: !!(props && props.uploading),
    uploadError: (props && props.uploadError) || null,
    onUpload: (props && props.onUpload) || (() => {}),
    // Human-in-the-loop BOM review (see App.jsx + ExtractedPanel).
    ...
    editBomItem: (props && props.editBomItem) || (() => {}),
    addBomItem: (props && props.addBomItem) || (() => {}),
    deleteBomItem: (props && props.deleteBomItem) || (() => {}),
