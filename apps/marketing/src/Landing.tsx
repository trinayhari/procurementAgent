...
function Cta({ variant, style, children }: { variant: 'primary' | 'ghost'; style?: string; children: ReactNode }) {
  ...
  return DEMO_URL
    ? <a className={className} href={DEMO_URL} style={css(style)}>{children}</a>
    : <button type="button" className={className} style={css(style)}>{children}</button>
}
  ...
  return (
    <div className="pq-page" style={css('background:var(--color-bg);color:var(--color-text)')}>
      ...
      <div className="pq-shell">
        ...
        <section style={css('padding:112px 0 84px')}>
          <h1 style={css('font-family:var(--font-heading);font-weight:800;font-size:clamp(42px,6.2vw,84px);line-height:clamp(44.5px,6.57vw,89px);letter-spacing:-0.02em;margin:0 0 0 -0.058em;text-box:trim-both cap alphabetic')}>
            {headline.map((line) => <span key={line} style={css('display:block')}>{line}</span>)}
          </h1>
          <p style={css('font-size:17px;line-height:28px;max-width:58ch;margin:34px 0 0;text-box:trim-both cap alphabetic')}>
          ...
          <div style={css('display:flex;gap:var(--space-3);flex-wrap:wrap;margin-top:28px')}>
            <Cta variant="primary">Book a demo</Cta>
        ...
        <section style={css('padding:84px 0 70px')}>
          <span style={css(KICKER)}>The week that isn't building</span>
          <h2 style={css(`${H2};margin:0 0 42px;max-width:26ch`)}>A project engineer's week, before and after.</h2>
          <div style={css('display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:28px 72px')}>
            <div>
              ...
              <ul style={css('list-style:none;margin:0;padding:0;display:flex;flex-direction:column;gap:0')}>
                {BEFORE.map((item, i) => (
                  <li
                    key={item}
                    style={css(
            ...
            <div>
              ...
              <p style={css(`${BODY};margin:20px 0 0;max-width:44ch`)}>
        ...
        <section id="screen" style={css('border-top:2px solid var(--color-divider);background:var(--color-surface);padding:70px 0 84px')}>
          <div className="pq-shell">
            <div style={css('display:flex;justify-content:space-between;align-items:baseline;gap:28px;flex-wrap:wrap;margin-bottom:28px')}>
              ...
              <p style={css(`${BODY};margin:0;max-width:46ch`)}>
            ...
            <p style={css('font-size:13px;line-height:28px;color:color-mix(in srgb,var(--color-text) 70%,transparent);margin:14px 0 0')}>
      ...
      <div className="pq-shell">
        ...
        <section id="vision" style={css('padding:84px 0 70px')}>
          <span style={css(KICKER)}>Procurement is the wedge</span>
          <h2 style={css('font-family:var(--font-heading);font-weight:800;font-size:clamp(32px,3.6vw,44px);line-height:1.1;letter-spacing:-0.015em;margin:0 0 0 -0.03em;max-width:30ch;text-box:trim-both cap alphabetic')}>
            Every operational workflow in a construction company ends up here.
          </h2>
          <p style={css('font-size:17px;line-height:28px;max-width:58ch;margin:34px 0 42px')}>
          ...
          <div style={css('display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:0 clamp(24px,4vw,56px)')}>
            {SURFACES.map((s, i) => (
              <div
                key={s.title}
                style={css(
      ...
      <section style={css('background:var(--color-accent);color:var(--color-bg)')}>
        <div style={css('max-width:1200px;margin:0 auto;padding:84px var(--edge)')}>
          <h3 style={css('font-family:var(--font-heading);font-weight:800;font-size:clamp(34px,4.2vw,56px);line-height:clamp(36px,4.45vw,59.4px);letter-spacing:-0.015em;margin:0 0 0 -0.058em;text-box:trim-both cap alphabetic')}>
          ...
          <div style={css('display:flex;gap:var(--space-3);flex-wrap:wrap;margin-top:42px')}>
